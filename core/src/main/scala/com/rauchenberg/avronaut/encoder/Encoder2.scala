...
case class Value[A](a: A)                               extends Result[A]
case class Record[A](genericRecord: GenericData.Record) extends Result[Nothing]
...
object Encoder {
  ...
  def encode[A](a: A)(implicit encoder: Encoder[A], schema: AvroSchema[A]): Either[Error, GenericData.Record] =
    schema.data.flatMap { schema =>
      safe(encoder.apply(a, schema) match {
        case gr: GenericData.Record => gr
  ...
  def combine[A](ctx: CaseClass[Typeclass, A]): Typeclass[A] =
    new Typeclass[A] {
      ...
      type Ret = GenericData.Record

      override def apply(value: A, sd: SchemaData): Ret =
                ...
                ctx.parameters.toList
                  ...
                  .map { param =>
                    ...
                    param.typeclass.apply(paramValue, sd) match {
                      ...
                      case Value(a)  => gr.put(i, a)
                      case Record(a) => gr.put(i, a)
  ...
  implicit def mapEncoder[A](implicit aEncoder: Encoder[A]): Encoder[Map[String, A]] = new Encoder[Map[String, A]] {
    override type Ret = Map[String, aEncoder.Ret]
    override def apply(value: Map[String, A], schemaData: SchemaData): Ret = value.map { case (k, v) =>
      k -> aEncoder(v)
    }
  }
