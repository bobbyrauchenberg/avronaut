...
//
//case class UnionRecord[T](genericRecord: GenericData.Record,
//                          fieldName: String,
//                          schema: Schema,
...
//
//    val findSubSchema = union.schema.getTypes.asScala
//      .find(matchFullName)
//      .headOption
//      .toRight(
...
////        .traverse(f => safe(gr.put(f.name, union.genericRecord.get(f.name))))
////        .map(_ => gr)
////    }
//
////    for {
...
//    //decodedT  <- subType.typeclass.decodeFrom(subRecord).map(_.asInstanceOf[T])
////    } yield null
//    null
//  }
//
...
//    ctx.subtypes
//      .filter(_.typeName.full == typeToMatch)
//      .headOption
//      .toRight(
