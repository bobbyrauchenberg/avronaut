package com.rauchenberg.avronaut.common.annotations
...
object SchemaAnnotations {
  ...
  def getNameAndNamespace(annotations: Option[SchemaMetadata],
                          defaultName: String,
                          defaultNamespace: String): (String, String) = {
  ...
  implicit class AnnotationValue(val annotations: Option[SchemaMetadata]) extends AnyVal {
    ...
    def namespace(default: String) = annotations.flatMap(_.values.get(Namespace)).getOrElse(default)
    def isNamespaceDefined         = annotations.flatMap(_.values.get(Namespace)).isDefined
  }
